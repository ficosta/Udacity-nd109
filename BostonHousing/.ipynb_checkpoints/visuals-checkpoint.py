...
def ModelLearning(X, y):
    ...
    train_sizes = np.rint(np.linspace(1, X.shape[0]*0.75 - 1, 9)).astype(int)
    ...
    for k, depth in enumerate([1, 3, 6, 10]):
        ...
        sizes, train_scores, test_scores = curves(
...
def ModelComplexity(X, y):
    ...
    ), X, y, param_name="max_depth", param_range=max_depth, cv=cv, scoring='r2')
